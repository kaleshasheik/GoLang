...
func main(){
    ...
    fmt.Println("")
    fmt.Println("")
    longest:=FindLongestWord(strArray)
    palindromeStrings:=CheckPalindrome(strArray)
    fmt.Println("Received longest word from channel: ",<-longest)
    fmt.Println("Received palindrome strings from channel: ",<-palindromeStrings)

    }

    func FindLongestWord(strArray []string) <-chan string {
...
func CheckPalindrome(strArray []string) <-chan []string {
